...
for jj = 1:size(sondefilename,2)
   ...
   [xx(jj,:), yy(jj,:), range_grid] = Sonde_read_M2HATS_nc_files(jj, elevation, sondedir, sondefilename,  N_avg_comb, duration, range_grid_size, range_grid_in, comb_AH_var, sonde_end_int, plot_path, flag); 
...
text(0.5, WV_max-1.5, ['y = ' num2str(fit(1),3) '*x + ' num2str(fit(2),3)], 'FontSize', 22, 'Color', 'r')
text(0.5, WV_max-2, ['Corr = ' num2str(Corr,3)], 'FontSize', 22, 'Color', 'r')
text(0.5, WV_max-2.5, ['StDev = ' num2str(StDev,3) 'g m^{-3}'], 'FontSize', 22, 'Color', 'r')
