...
% Store the True Systematic Error for comparison (if no correction is applied)
 N_off_raw_true = N_off_true + N_AP_decay_zero;
 N_on_raw_true = N_on_true + N_AP_decay_zero;

% Use raw signal directly for log ratio.
...
% A systematic error inherent to pulse smearing occurs when using the log-ratio technique.
...
for i = 1:length(kappa_test_values)
    ...
    N_AP_sub = kappa * N_AP_decay_zero; 
    
    % Create the RAW Contaminated Signal (N_true + N_AP_true)
    N_off_raw = N_off_true + N_AP_decay_zero;
    N_on_raw = N_on_true + N_AP_decay_zero;
