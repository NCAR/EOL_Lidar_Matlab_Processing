...
% showing the median and a 3-sigma shaded area over a 1-hour window.
    ...
    [~, idx_closest_single] = min(abs(x - target_time));
    ...
    time_indices = find(x >= start_time - TOLERANCE & x <= end_time + TOLERANCE);
    ...
    if num_profiles_found < 2
        ...
        time_str_range = ['Single Profile: ', datestr(x(idx_closest_single), 'dd-mmm-yyyy HH:MM:SS UTC')];
    else
        time_str_range = [datestr(x(idx_to_use(1)), 'dd-mmm-yyyy HH:MM') ' to ' datestr(x(idx_to_use(end)), 'HH:MM UTC')];
    end
...
% manual median filter to smooth data anomalies.
    ...
    AH_surf_filt = manual_medfilt1(AH_surf, FILTER_ORDER);
    ...
    plot(x, AH_MP_lowest_filt, 'b--', 'LineWidth', 1.5, 'DisplayName', 'MultiPulse Lowest Bin (Filtered)');
