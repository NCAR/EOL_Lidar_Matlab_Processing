...
flag.plot_profile_lines = 1;   % Toggle vertical profile line plots for a specific time/date

% --- PROFILE PLOT SETTINGS ---
PROFILE_TIME_TO_PLOT = datenum('2025-07-29 24:00:00', 'yyyy-mm-dd HH:MM:SS'); 
% -----------------------------
...
if flag.plot_profile_lines
    create_profile_plots(comb_duration, y, comb_AH, comb_AH_PTV, comb_AH_MultiPulse, comb_AH_model, ...
        comb_T, comb_T_Std, comb_T_model, comb_AH_surf, comb_T_surf, ...
...
function create_profile_plots(x, y_alt, AH, AH_PTV, AH_MultiPulse, AH_model, ...
...
% CREATE_PROFILE_PLOTS Generates line plots of vertical profiles at a specific time.
    ...
    % Find the closest index in the time series to the target time
    idx = find_closest_time_index(x, target_time);
    
    time_str = datestr(x(idx), 'dd-mmm-yyyy HH:MM:SS UTC');
    
    % Extract the specific profile data and surface data
    AH_Std_profile = AH(:, idx);
    AH_PTV_profile = AH_PTV(:, idx);
    AH_MultiPulse_profile = AH_MultiPulse(:, idx);
    AH_Model_profile = AH_model(:, idx);
    T_PTV_profile = T(:, idx);
    T_Std_profile = T_Std(:, idx);
    T_Model_profile = T_model(:, idx);
    
    AH_Surf_val = AH_surf(idx);
    T_Surf_val = T_surf(idx);
    
    % --- FIGURE 1: ABSOLUTE HUMIDITY PROFILES ---
    hf1 = figure(figure_idx);
    set(hf1, 'Position', [50 50 500 700]);
    plot(AH_Std_profile, y_alt, 'b-', 'LineWidth', 2, 'DisplayName', 'AH Standard'); hold on;
    plot(AH_PTV_profile, y_alt, 'r-', 'LineWidth', 2, 'DisplayName', 'AH PTV');
    plot(AH_MultiPulse_profile, y_alt, 'g-', 'LineWidth', 2, 'DisplayName', 'AH MultiPulse');
    plot(AH_Model_profile, y_alt, 'k--', 'LineWidth', 2, 'DisplayName', 'AH ERA5 Model');
    
    % Plot Surface Station Data at 0 km
    plot(AH_Surf_val, 0, 'ms', 'MarkerSize', 10, 'MarkerFaceColor', 'm', 'DisplayName', 'AH Surface');

    grid on; box on;
    title({[node, ' Absolute Humidity Profile'], ['Time: ', time_str]}, 'fontweight', 'b');
    ...
    plot(T_PTV_profile, y_alt, 'r-', 'LineWidth', 2, 'DisplayName', 'T PTV'); hold on;
    plot(T_Std_profile, y_alt, 'b-', 'LineWidth', 2, 'DisplayName', 'T Standard');
    plot(T_Model_profile, y_alt, 'k--', 'LineWidth', 2, 'DisplayName', 'T ERA5 Model');
    
    % Plot Surface Station Data at 0 km
    plot(T_Surf_val, 0, 'ms', 'MarkerSize', 10, 'MarkerFaceColor', 'm', 'DisplayName', 'T Surface');

    grid on; box on;
    title({[node, ' Temperature Profile'], ['Time: ', time_str]}, 'fontweight', 'b');
