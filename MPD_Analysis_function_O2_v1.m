...
if flag.afterpulse == 1   % afterpulse correction
  ...
  %  read my version of the afterpulse calibration files which are now using rates     
      ...
      load (afterpulse_filename, 'ap_O2_comb_off', 'ap_O2_comb_on');  
      load (afterpulse_filename, 'ap_O2_mol_off', 'ap_O2_mol_on');
      if flag.molecular == 1
        ap_spline_sub_off = ap_O2_mol_off*MCS.accum*MCS.bin_duration*1e-9;
        ap_spline_sub_on = ap_O2_mol_on*MCS.accum*MCS.bin_duration*1e-9;
      else
        ap_spline_sub_off = ap_O2_comb_off*MCS.accum*MCS.bin_duration*1e-9;
        ap_spline_sub_on = ap_O2_comb_on*MCS.accum*MCS.bin_duration*1e-9;
      end
  else    
...
% use to compare technique from above
...
%     Offline_Temp_Spatial_Avg(FX<-(3000*(MCS.bin_duration*MCS.accum)/500/14200)) = nan; % remove falling edge of clouds
%     Offline_Temp_Spatial_Avg(FX> (3000*(MCS.bin_duration*MCS.accum)/500/14200)) = nan; % remove leading edge of clouds   
