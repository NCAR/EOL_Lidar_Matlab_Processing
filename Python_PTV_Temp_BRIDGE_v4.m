...
% ADDED: Temperature_Standard, Temperature_Standard_mask, Temperature_Standard_uncertainty
variables = cell(1, 41);
variables([1:12, 13:15, 16:18, 26:28, 30:32, 40:41]) = {
    ...
    'Temperature_Standard', 'Temperature_Standard_mask', 'Temperature_Standard_uncertainty', ... % <-- NEW VARIABLES AT INDICES 13-15
...
for jj = 1:num_files
    ...
    % Absolute Humidity (Standard, PTV, MultiPulse)
...
comb_T_Std = nan(num_ranges, total_timesteps); % <-- NEW
comb_T_Std_var = nan(num_ranges, total_timesteps); % <-- NEW
...
for jj = 1:num_files
    % CRITICAL: Calculate current_timesteps based on a 2D array's column count
    ...
    comb_T_Std(:, start_col:end_col) = T_Std{jj}; % <-- NEW
    comb_T_Std_var(:, start_col:end_col) = T_Std_var{jj}; % <-- NEW
...
% ADDED NEW ARRAYS to the data arrays lists
data_arrays_2d = {'comb_AH', 'comb_AH_var', 'comb_AH_PTV', 'comb_AH_PTV_var', 'comb_AH_MultiPulse', 'comb_AH_MultiPulse_var', 'comb_AH_model', 'comb_ABC', 'comb_ABC_var', 'comb_T', 'comb_T_var', 'comb_T_Std', 'comb_T_Std_var', 'comb_T_model', 'comb_Counts'};
...
% --- 6. PLOT SETUP AND CALLS (Updated for new model temperature plot) ---
...
% TEMPERATURE (PTV/STANDARD/MODEL)
create_pcolor_plot(x, y, comb_T, 'Temp, PTV (K)', caxis_T_abs, 'plasma', node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_T_Std, 'Temp, Standard (K)', caxis_T_abs, 'plasma', node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_T_model, 'Temp, ERA5 Model (K)', caxis_T_abs, 'plasma', node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1; % <-- NEW PLOT

% AEROSOL/COUNTS
create_pcolor_plot(x, y, comb_ABC, 'Aerosol Backscatter Coefficient, PTV (m^{-1} sr^{-1})', caxis_ABC, 'viridis', node, plot_size1, font_size, xData, xData_m, figure_idx, 1); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_Counts, 'Attenuated Backscatter, 828 nm', caxis_Counts, 'jet', node, plot_size1, font_size, xData, xData_m, figure_idx, 1); figure_idx=figure_idx+1;

% ABSOLUTE HUMIDITY (ABS)
create_pcolor_plot(x, y, comb_AH, 'Absolute Humidity Standard (g m^{-3})', caxis_AH, cmap_AH, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_PTV, 'Absolute Humidity PTV (g m^{-3})', caxis_AH, cmap_AH, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_MultiPulse, 'Absolute Humidity MultiPulse (g m^{-3})', caxis_AH, cmap_AH, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_model, 'Absolute Humidity ERA5 Model (g m^{-3})', caxis_AH, cmap_AH, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;

% ABSOLUTE HUMIDITY (DIFF)
create_pcolor_plot(x, y, comb_AH-comb_AH_model, 'Absolute Humidity Standard-ERA5 (g m^{-3})', caxis_diff_AH, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_PTV-comb_AH_model, 'Absolute Humidity PTV-ERA5 (g m^{-3})', caxis_diff_AH, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_MultiPulse-comb_AH_model, 'Absolute Humidity, MultiPulse-ERA5 (g m^{-3})', caxis_diff_AH, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;

% TEMPERATURE (DIFF)
create_pcolor_plot(x, y, comb_T-comb_T_model, 'Temperature, PTV-ERA5 (K)', caxis_diff_T, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_T_Std-comb_T_model, 'Temperature, Standard-ERA5 (K)', caxis_diff_T, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1; % <-- NEW PLOT

% UNCERTAINTY
create_pcolor_plot(x, y, comb_AH_var, 'Absolute Humidity Standard Uncertainty (g m^{-3})', caxis_uncertainty, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_PTV_var, 'Absolute Humidity PTV Uncertainty (g m^{-3})', caxis_uncertainty, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_AH_MultiPulse_var, 'Absolute Humidity MultiPulse Uncertainty (g m^{-3})', caxis_uncertainty, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1;
create_pcolor_plot(x, y, comb_T_Std_var, 'Temperature Standard Uncertainty (K)', caxis_uncertainty, smooth_gray_cmap, node, plot_size1, font_size, xData, xData_m, figure_idx, 0); figure_idx=figure_idx+1; % <-- NEW PLOT
...
% --- 1D HISTOGRAMS (Figures 20, 21) ---
create_1d_histogram(comb_T - comb_T_model, 'T PTV - ERA5 Model Difference', 'Temperature Difference (K)', T_hist_limits, figure_idx, node, font_size); figure_idx=figure_idx+1;
create_1d_histogram(comb_AH - comb_AH_model, 'AH Standard - ERA5 Model Difference', 'Absolute Humidity Difference (g m\textsuperscript{-3})', AH_hist_limits, figure_idx, node, font_size); figure_idx=figure_idx+1;

% --- 2D DENSITY MAPS (Figures 22-26) ---
create_2d_density_map(comb_T - comb_T_model, y_range, ...
    'T PTV - ERA5 Difference vs. Range (Bin Width: 0.2 K)', 'Temperature Difference (K)', ...
    T_diff_bin_limits, [0 6], T_diff_bin_width, figure_idx, node, plot_size1, font_size, cmap_density); figure_idx=figure_idx+1;

create_2d_density_map(comb_T_Std - comb_T_model, y_range, ... 
...
create_2d_density_map(comb_AH - comb_AH_model, y_range, ...
    ...
    AH_diff_bin_limits, [0 6], AH_diff_bin_width, figure_idx, node, plot_size1, font_size, cmap_density); figure_idx=figure_idx+1;
...
plot_size_square = [100 100 750 750]; 
num_plots = figure_idx - 1;

filename_suffixes = {
    'T_PTV_multi', 'T_Standard_multi', 'T_Model_multi', ... % <-- NEW
    'Back_Coeff_PTV_multi', '828_Counts_multi', ...
    'WV_Standard_multi', 'WV_PTV_multi', 'WV_MultiPulse_multi', 'WV_Model_multi', ...
    'WV_Diff_Standard_Model', 'WV_Diff_PTV_Model', 'WV_Diff_MultiPulse_Model', ...
    'T_Diff_PTV_Model', 'T_Diff_Standard_Model', ... 
    'WV_Standard_Uncertainty', 'WV_PTV_Uncertainty', 'WV_MultiPulse_Uncertainty', 'T_Standard_Uncertainty', ...
    'T_Diff_Histogram', 'AH_Diff_Histogram', ...
    'T_Diff_PTV_Histogram_Range', 'T_Diff_Standard_Histogram_Range', ... 
    'AH_Diff_Standard_Histogram_Range', ...
...
for k = 1:num_plots
    ...
    drawnow;
    FigH.Units = 'pixels';
    % Set position based on figure type (Figures 20 and later are square)
    if k >= 20 
