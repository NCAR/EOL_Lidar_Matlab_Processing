function[] = DIAL_Analysis_v53_function(folder_in, MCS, write_data_folder, flag, node, ...
...
hitran = dlmread('770nm_841nm_HITRAN_2008.csv', ',',[1 1 3027 8]);
...
% experiment to use HSRL combined channel for offline data
folder_NF = strcat(strtok(folder_in, 'FF'), 'NF')  
[~,data_comb,folder_NF] = File_Retrieval_v13(MCS.bins, folder_NF);
...
try
    ...
    Combined_Raw_Data = data_comb(1:size(Online_Raw_Data,1),1:end);
catch err
    Offline_Raw_Data = data_off(1:size(data_off,1),1:end);
    Online_Raw_Data = data_on(1:size(Offline_Raw_Data,1),1:end);
    Combined_Raw_Data = data_comb(1:size(Offline_Raw_Data,1),1:end);
end
...
% Parsing off the auxiliary 
 ...
 figure(290)
 plot(mean(Offline(1:10000,:)))
 hold on
 Combined = single(Combined_Raw_Data(:,8:end)).*(780/828);
 plot(mean(Combined(1:10000,:)))
 hold off
 
 figure(291)
 O = nanmean(Combined(1:10000,:))./nanmean(Offline(1:10000,:));
 plot(O)
 
 figure(292)
 Combined_corrected = bsxfun(@rdivide, Combined, O); 
 plot(mean(Offline(1:7500,:)))
 hold on
 plot(mean(Combined_corrected(1:7500,:)))
 hold off
 
 Offline = Combined_corrected; %use the 780nm HSRL combined channel corrected for overlap function
...
%WNmax = 1/(776)*1e7;  % 824.0 nm converted to wavenumber
