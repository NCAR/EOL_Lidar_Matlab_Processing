%% MicroPulse DIAL Afterpulse Error Simulation (Final Consistent Model)
...
sigma_on_cm2 =0.5e-23;      
sigma_off_cm2 = 7.0e-25;    
...
% Range Vector (Sampled at 150m intervals)
R = (R_start : dR : R_end)';
N_bins = length(R);

% Midpoint range vector for plotting 
...
% True number online and offline counts (The final, physically-resolved signal)
...
%% Detector Contamination Model (Instantaneous AP Added)

N_AP_peak = C_sys * AP_fraction;

% N_AP_decay is the instantaneous profile 
N_AP_decay = N_AP_peak * (A_1*exp(-(R - R_start)/lambda_AP_1)+ (A_2*exp(-(R - R_start) / lambda_AP_2)));
N_AP_total = N_AP_decay + N_dark;

% Contaminated signal profiles (Smoothed N_true + Instantaneous N_AP)
N_off_raw = N_off_true + N_AP_decay;
N_on_raw = N_on_true + N_AP_decay;


%% Full Numerical Retrieval of contaminated signals (No Digital Filtering Required)

% The AP contamination is present, but NO CORRECTION is applied here to calculate 
% the error caused by the uncorrected AP.

% Apply moving average filter to the raw signals
N_off_raw_smooth = movmean(N_off_raw, M);
N_on_raw_smooth = movmean(N_on_raw, M);

% Calculate the contaminated log ratio (using raw signals)
...
WV_OD_4km = (OD_on(R_4km_idx) - OD_off(R_4km_idx));
disp('-------------------------------------------------------------------');
disp(['One-way WV OD at 4 km: ' num2str(WV_OD_4km)]);
disp('-------------------------------------------------------------------');
...
figure('Position', [100, 600, 1000, 600]);
