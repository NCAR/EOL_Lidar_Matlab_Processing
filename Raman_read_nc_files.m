...
filename = strcat(serv_path,'spuler/mpd/sgp/raman_lidar/sgprlprofmr2news10mC1.c0.20190426.000500.nc');
...
ncdisp(filename) % use this to display all variables
 ...
 Raman_time = ncread(filename,variable{2}); 
 Raman_alt = ncread(filename,variable{3});
 Raman_MR = ncread(filename,variable{4}); 
 temp_sonde = ncread(filename,variable{5}); 
 pres_sonde = ncread(filename,variable{6}); 
 
 netcdf.close(ncid);
 
%convert from Unix time to date number (days since Jan 0 0000) 
Raman_duration = datenum(datetime(int64(Raman_time) + int64(base_time), 'convertfrom', 'posixtime'));
...
density = pres_sonde./(8.31447E-2*temp_sonde)*28.97;
Raman_AH = density.*Raman_MR.*1e-3; %to convert from kg/m# to g/m^3
