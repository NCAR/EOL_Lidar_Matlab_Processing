function[] = DIAL_Analysis_function_NetCDF_v2(folder_in, date_in, MCS, write_data_folder, flag, node, wavemeter_offset,...
...
%% read in weather station data
  ...
  P_on = Online_Raw_Data(:,4); % transmitted average power
  P_off = Offline_Raw_Data(:,4); % transmitted average power
  T_bench = Online_Raw_Data(:,5); % optical bench temperature (thermocouple 1)
  T_base = Offline_Raw_Data(:,5); % base temperature (thermocouple 2)
  
if flag.WS==1
  % read in surface weather station data
  Surf_T = Offline_Raw_Data(:,6);  %temperature in C
  Surf_P = Offline_Raw_Data(:,7)./1013.249977;  % pressure in atm
  Surf_RH = Offline_Raw_Data(:,8);
...
lambda_on_all=Online_Raw_Data(:,2);
lambda_on_diff= Online_Raw_Data(:,3);
lambda_off_all=Offline_Raw_Data(:,2);
lambda_off_diff= Offline_Raw_Data(:,3);

offset = -wavemeter_offset*1e9; % This is the measured correction (nm) for each unit 
lambda_on_all = lambda_on_all+offset;
lambda_off_all = lambda_off_all+offset;

ii=1;
for ii=1:size(lambda_on_all,1)
    if ii>1&& lambda_on_all(ii)<828
        lambda_on_all(ii)=lambda_on_all(ii-1);
        lambda_on_diff(ii)=lambda_on_diff(ii-1);
    end
    if ii>1&& lambda_off_all(ii)<828
        lambda_off_all(ii)=lambda_off_all(ii-1);
        lambda_off_diff(ii)=lambda_off_diff(ii-1);
    end
end

lambda_on = median(lambda_on_all)
lambda_off = median(lambda_off_all)
...
% end
    ...
    edges_on=828.1805+offset:.001:828.2205+offset;
    mult_fac = 1;
    [value,edges]=histcounts(round(mult_fac*lambda_on_all,3)/mult_fac,edges_on); % bin rounded wavelengths
    lambda_on_N = edges(value>=1000)  % wavelength values with occurance > 10
    %lambda_F = value(value~=0);  % frequency of occurance
    lambda_all_N = round(mult_fac*lambda_on_all,3)/mult_fac; 
    figure(1234)
    plot(lambda_all_N)
    hold on
    plot(lambda_on_all)
    ...
    edges_off=828.280:.001:828.320;
    mult_fac = 1;
    [value,edges]=histcounts(round(mult_fac*lambda_off_all,3)/mult_fac,edges_off); % bin rounded wavelengths
    ...
    lambda_all_off_N=round(mult_fac*lambda_off_all,3)/mult_fac;
    figure(5678)
...
% Parsing off the auxiliary 
 Online = single(Online_Raw_Data(:,9:end)); 
 Offline = single(Offline_Raw_Data(:,9:end));
...
% h = waitbar(0,'Temporal and Spatial Averaging');
 ...
 % regular averaging
  ...
  lambda_on_all = interp1(time, lambda_on_all, time_grid, 'next', extrapolation); 
  ...
  lambda_off_all = interp1(time, lambda_off_all, time_grid, method, extrapolation);
...
%% Mask the Number density data based on the error, correct for range center, and add WS data at lowest gate 
 ...
 check_high_counts = Offline_Raw_Data(:,8:end)./(MCS.bin_duration*1e-9*MCS.accum);
...
if flag.mark_gaps ==1
  ...
  r_blank = bsxfun(@times, ones(length(lambda_on_all),length(range)), lambda_blank);
  lambda_off_all(isnan(lambda_blank)) = NaN; 
...
if flag.decimate == 1 
    ...
    lambda_on_all = lambda_on_all(1:decimate_time:end);
    if flag.mark_gaps ==1
     lambda_blank = lambda_blank(1:decimate_time:end);
    end
    lambda_off_all = lambda_off_all(1:decimate_time:end);
