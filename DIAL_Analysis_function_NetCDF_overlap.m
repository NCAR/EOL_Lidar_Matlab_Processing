...
%% read in weather station data
  ...
  T_bench = Online_Raw_Data(:,5); % transmitted average power
  T_base = Offline_Raw_Data(:,5); % transmitted average power
...
% h = waitbar(0,'Temporal and Spatial Averaging');
 ...
 % regular averaging
  ...
  T_bench = interp1(time, T_bench, time_grid, method, extrapolation);
  T_base = interp1(time, T_base, time_grid, method, extrapolation);
  if flag.WS == 1
