function[] = DIAL_Analysis_function_NetCDF_v3(folder_in, date_in, MCS, write_data_folder, flag, node, wavemeter_offset,...
...
if flag.near == 1
  [data_on,data_off,near_on, near_off, folder_in] = File_Retrieval_NetCDF_v4(flag, MCS.bins, folder_in); %use to read binary data (bin number passed in) 
   % this is a way to process the near range data instead of the normal
   data_on = near_on;
   data_off = near_off;
   blank_range = 150; % new pulse generator shifts gate timing so less outgoing pulse contamination 
else
...
% h = waitbar(0,'Temporal and Spatial Averaging');
 ...
 range = single(0:gate:(size(Online,2)-1)*gate); % set range back to normal grid spacing
 ...
 RB_on = interp1(range_act, RB_on_act', range, method, extrapolation)';
 RB = interp1(range_act, RB_act', range, method, extrapolation)';
